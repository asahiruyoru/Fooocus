...
| Colab 対応 | `notebooks/fooocus_anima_colab.ipynb` から `wai_anima` preset をそのまま起動可能 |
| Balanced モード | notebook 側で `balanced` / `custom_steps` の派生 preset を生成可能 |
| 起動修正 | `rembg` を遅延ロード化し、オプション依存が無くても起動可能に |

### ControlNet: AnyTest 対応

- ControlNet モデルを **PyraCanny / CPDS** から **AnyTest / AnyTest_A / AnyTest_B** まで拡張
- Illustrious 向けに **AnyTest_A / AnyTest_B** の個別モデルを選択可能
- Classic 系 (PyraCanny / CPDS) と AnyTest 系を**同時に利用可能**
- ControlNet スロットを増設し、スロットごとに異なるモデルタイプを割り当て可能
- PyraCanny / CPDS / AnyTest 系は非正方形入力でも中央クロップせず、入力全体を保持したまま前処理を適用
- `Image Prompt` タブでは、非正方形の ControlNet 入力に合わせて生成解像度も自動追従
- 各スロットに **Use Square Crop** チェックボックスを追加。旧来の中央正方形クロップ挙動に戻す用途 (既定 OFF)
- WD14 Tagger を **v3 (wd-eva02-large-tagger-v3)** にアップグレード

### NoobAI Inpaint / Outpaint 専用タブ (新規)

Illustrious ベースの **NoobAI XL** 向け Inpaint/Outpaint ワークフローを専用タブとして追加しました。

- **NoobAI Inpaint タブ**: キャンバスに直接マスクを描いて部分修正。`手描き` / `矩形` ボタンでモードを切り替え、スケッチマスクと任意個数の矩形リージョンを同じ画像上に重ねて指定可能 (選択中の矩形は `\` キーで削除。レターボックス表示時も矩形が画像に正しく追従)
- **NoobAI Outpaint タブ**: Left / Right / Top / Bottom の拡張方向をチェックボックスで指定
- Inpaint engine に `noobai` を追加 (`NoobAI_Inpainting_ControlNet.safetensors` を初回利用時に自動ダウンロード)
- Inpaint head/patch モデルを使わず ControlNet ベースで動作 (NoobAI XL 専用設計)
- denoising strength / respective field などの共有設定は従来どおり Advanced → Inpaint で調整可能

### その他の UI 改善

- アスペクト比パネルにカスタム幅/高さ入力を追加
- 高解像度プリセット (1344 / 1360 / 1536 / 1664) を追加
- LoRA スロット数を **10 個** まで拡張
- メタデータ取り込みを修正: Gradio Image エディタの再エンコードで PNG/EXIF メタデータが欠落する問題に対し、アップロード時にパースした結果を `gr.State` に保持して回避

### Notebook / Colab

- Animayume ノートブックプロファイルを追加し、各種プリセットを整備
- T4/L4 クラス GPU では `--always-gpu` を既定の推奨設定に変更
...
> **注記**: このリポジトリは [lllyasviel/Fooocus](https://github.com/lllyasviel/Fooocus) → [mashb1t/Fooocus](https://github.com/mashb1t/Fooocus) からの**フォーク**であり、公式リポジトリではありません。
...
Windows 向けのビルド済みパッケージは [mashb1t/Fooocus のリリースページ](https://github.com/mashb1t/Fooocus/releases) から入手できます (本フォークのビルドではない点に注意)。

本フォークのソースを使う場合は、以下で git clone してください。
...
本 repo で管理している Colab notebook は `notebooks/` 配下のみで、現行の tracked notebook は `notebooks/fooocus_anima_colab.ipynb` です。既定では repo 内の `wai_anima` preset をそのまま利用します。
...
> T4/L4 クラスの GPU では `--always-gpu` を推奨 (ピーク VRAM 約 11.4GB)。VRAM が不足する場合は `--always-high-vram` にフォールバックしてください。
...
SDXL 版の Colab ノートブックは、元の [mashb1t/Fooocus](https://github.com/mashb1t/Fooocus) で提供されています。
...
WAI Illustrious v16 + NoobAI Inpaint 用には `wai_illustrious_v16_noobai` preset を用意しています (tracked notebook ではなく preset のみ提供)。必要に応じて `python entry_with_update.py --preset wai_illustrious_v16_noobai` で起動してください。
...
rembg によるマスク自動生成に対応。衣服カテゴリ別 / プロンプト別のセグメンテーションが利用できます。
...
adetailer に類似した機能で、動的画像検出ベースのワンクリック補正が可能です。[詳細](https://github.com/mashb1t/Fooocus/discussions/42)
...
**いいえ。** UI・タスクキュー・プリセットシステムはすべて従来の Fooocus のままです。
...
| ステップ数 | `30〜50` |
| CFG | `4〜5` |
| 解像度 | `1024x1024` (約1MP: 896x1152, 1152x896 等も可) |
...
**Positive:**
```
masterpiece, best quality, score_7, highres, safe, 1girl, solo, looking at viewer, smile, long hair, detailed eyes, detailed face, clean lines, smooth shading, soft lighting
```
...
初回起動後に生成される `config.txt` を編集して、モデルパスやデフォルトパラメータを変更できます。設定を壊してしまった場合は、`config.txt` を削除すればデフォルトに戻ります。
...
他のキーについては、初回起動後に生成される `config_modification_tutorial.txt` を参照してください。
</details>
...
| 方法 | フラグ | 備考 |
|------|--------|------|
| ローカル公開 | `--listen` (ポートは `--port 8888` 等で指定) | LAN 内アクセス |
| 外部公開 | `--share` | `.gradio.live` のエンドポイントを発行 |

認証を有効にするには、`auth.json` を作成してください ([auth-example.json](./auth-example.json) 参照)。
...
`language/` フォルダに翻訳用 JSON を配置し、`--language <名前>` を付けて起動すると UI が翻訳されます。
