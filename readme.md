...
| プリセット | `wai_anima` / `anima_preview3` / `anima_preview2` / `wai_illustrious_v16_noobai` を追加 |
...
- ControlNet モデルを **PyraCanny/CPDS → AnyTest / AnyTest_A / AnyTest_B** まで拡張
- Illustrious 向けに **AnyTest_A / AnyTest_B** の個別モデルを選択可能
- Classic と AnyTest の ControlNet を**同時利用可能**に
- ControlNet スロットを全タイプ分に増設
- PyraCanny / CPDS / AnyTest 系は非正方形入力でも中央クロップせず、入力全体を保持したまま ControlNet 前処理を適用
- `Image Prompt` タブでは、非正方形の ControlNet 入力を使うと生成解像度も入力サイズに追従
- 各 ControlNet スロットには旧来の中央正方形 crop に戻す **Use Square Crop** チェックボックスを追加 (既定では OFF)
...
- **NoobAI Inpaint タブ**: キャンバスに直接マスクを描いて部分修正
- **NoobAI Inpaint タブ**: 手描きマスクに加えて JSON で任意個数の回転矩形 region を追加可能
...
- メタデータ取り込みを修正: Gradio Image エディタの再エンコードで PNG/EXIF メタデータが消える問題を、アップロード時にパースした結果を `gr.State` に保持することで回避
...
- **Anima Preview3**: 実験的サポート (上級者向け、SDXL ワークフローの完全な代替ではありません。Preview2 も利用可能)
...
この repo で管理している Colab notebook は `notebooks/` 配下のみです。現行の tracked notebook は `notebooks/fooocus_anima_colab.ipynb` で、デフォルトでは repo 内の `wai_anima` preset をそのまま使います。

`wai_anima` preset は `1344x1344` / `60 steps` / `Quality` を基本値にし、checkpoint / Qwen text encoder / VAE を preset 経由で自動ダウンロードします。`balanced` と `custom_steps` を選んだときだけ notebook 側で派生 preset を一時生成します。
...
WAI Illustrious v16 + NoobAI Inpaint 用には `wai_illustrious_v16_noobai` preset を追加しています。これは tracked notebook ではなく preset 提供のみなので、必要に応じて `python entry_with_update.py --preset wai_illustrious_v16_noobai` で使ってください。
