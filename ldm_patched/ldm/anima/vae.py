...
from ldm_patched.ldm.modules.diffusionmodules.model import torch_cat_if_needed
from ldm_patched.modules import model_management
import ldm_patched.modules.ops as ldm_ops

ops = ldm_ops.disable_weight_init

CACHE_T = 2

def get_vae_attention():
    if model_management.xformers_enabled_vae():
        from ldm_patched.ldm.modules.diffusionmodules.model import xformers_attention
        return xformers_attention
    elif model_management.pytorch_attention_enabled():
        from ldm_patched.ldm.modules.diffusionmodules.model import pytorch_attention
        return pytorch_attention
    else:
        from ldm_patched.ldm.modules.diffusionmodules.model import normal_attention
...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.optimized_attention = get_vae_attention()
...
class WanVAE(nn.Module):
    ...
    def decode(self, z):
        ...
        out_res = torch.cat(out, 2)

        # 4D return fallback
        if is_2d:
            out_res = out_res.squeeze(2)
