...
from ldm_patched.ldm.cosmos.predict2 import MiniTrainDIT
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if self.use_self_attn:
            self.norm_self_attn = (
                operations.LayerNorm(model_dim, device=device, dtype=dtype)
                if layer_norm
                else operations.RMSNorm(model_dim, eps=1e-6, device=device, dtype=dtype)
            )
